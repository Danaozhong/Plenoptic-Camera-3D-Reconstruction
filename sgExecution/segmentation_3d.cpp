...
#include "sgBase/image.h"
#include "sgBase/misc/color.h"
#include "sgBase/misc/misc.h"
...
#include "sgExecution/image_preprocessing_2d.h"
#include "sgExecution/find_contours_2d.h"

#include "sgExecution/contour_fitting_3d.h"
#include "sgExecution/segmentation_3d.h"
...
namespace sgExecution
{
		...
		:plenopticCamera(cameraParameterFileName), showResults(true)
	{
		...
		this->depthImage = cv::imread(depthImageFileName, CV_LOAD_IMAGE_ANYDEPTH);
	...
	std::vector<Contour3D> Segmentation3D::ExecuteDepthFitting(ContourFitting3D contourFitting, Segmentation3DDepthFittingMethod depthFittingMethod, double radius, int id)
	{
		...
		for (int i = 0; i != currentContours3D.size(); i++)
		{
		...
		for (int i = 0; i != currentContours3D.size(); i++)
		{
			...
			for (int k = 0; k != currentContours3D[i].contourLines.size(); k++)
			{
	...
	int Segmentation3D::ExecuteContourFittingApproach(Segmentation3DContourDetectionMethod lineDetectionMethod, Segmentation3DDepthFittingMethod depthFittingMethod)
	{
		...
		// perform canny 
		{
			...
			boost::chrono::thread_clock::time_point contourDetectionStartTime = boost::chrono::thread_clock::now();

			this->contourImage = ImagePreprocessing2D::CreateContourImageUsingCanny(this->colorImage, 35);
			boost::chrono::thread_clock::time_point contourDetectionEndTime = boost::chrono::thread_clock::now();
			std::cout << "Canny edge detection complete (duration " << boost::chrono::duration_cast<boost::chrono::milliseconds>(contourDetectionEndTime - contourDetectionStartTime).count() << " ms)." << std::endl;
		}
...

		{
			...
			boost::chrono::thread_clock::time_point contourDetectionEndTime = boost::chrono::thread_clock::now();
			std::cout << "Contour detection complete (duration " << boost::chrono::duration_cast<boost::chrono::milliseconds>(contourDetectionEndTime - contourDetectionStartTime).count() << " ms)." << std::endl;
		}
		...
		if (true == this->showResults)
		{
			...
			for (int i = 0; i != this->contours2D.size(); i++)
			{
				Color currentColor = ColorHelper::GetColorFromIndex(i);
				for (int k = 0; k != this->contours2D[i].contourLines.size(); k++)
				{
					...
					cv::line(colorImageWithLines, p1, p2, cv::Scalar(currentColor.r(), currentColor.g(), currentColor.b()), 2, CV_AA);
				}
			}
...
#else
		{
			...
			boost::chrono::thread_clock::time_point contourDetectionEndTime = boost::chrono::thread_clock::now();
			std::cout << "Depth fitting complete (duration " << boost::chrono::duration_cast<boost::chrono::milliseconds>(contourDetectionEndTime - contourDetectionStartTime).count() << " ms)." << std::endl;
		}
#endif
		
		{
			...
			for (int i = 0; i != this->contours3D.size(); i++)
			{
				this->contours3D[i].PostprocessContours(this->matrixOfIntrinsicParameters);
			}

			boost::chrono::thread_clock::time_point contourDetectionEndTime = boost::chrono::thread_clock::now();
			std::cout << "Contour postprocessing complete (duration " << boost::chrono::duration_cast<boost::chrono::milliseconds>(contourDetectionEndTime - contourDetectionStartTime).count() << " ms)." << std::endl;
		}

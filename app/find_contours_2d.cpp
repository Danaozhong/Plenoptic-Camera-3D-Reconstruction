...
#include "base/geometrics/quadtree.h"
...
namespace sgExecution
{
	...
	std::vector<Contour2D> FindContours2D::FindContoursSuzukiAbe()
	{
		...
		cv::findContours(this->edgeImage, contours, hierarchy, cv::RetrievalModes::RETR_LIST, cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0));

		size_t minNumOfPoints = 85u;
		...
		for (size_t i = 0; i < filteredContours.size(); i++)
		{
			...
			for (size_t k = 0; k != filteredContours[i].size() - 1; k++)
			{
				Eigen::Vector3f p1;
				Eigen::Vector3f p2;
	...
	std::vector<Contour2D> FindContours2D::FindContoursRANSAC2D()
	{
		...
		cv::findContours(this->edgeImage, contours, hierarchy, cv::RETR_LIST, cv::CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
	...
	std::vector<Contour2D> FindContours2D::FindContoursRANSAC2DWithQuadtree()
	{
		...
		for (size_t i = 0; i != quadtree.leafNodes.size(); i++)
		{
			pointDensityVariances.push_back(QuadtreeHelper::QuadtreeLeafNodePointDensityVariance(*(quadtree.leafNodes[i])));
			std::cout << "Point  density variance node " << i << ": " << QuadtreeHelper::QuadtreeLeafNodePointDensityVariance(*(quadtree.leafNodes[i])) << std::endl;
		}
	...
	std::vector<Contour2D> FindContours2D::FindContoursHoughTransform()
	{
		...
		cv::HoughLinesP(this->edgeImage, lines, 1, M_PI / 180, 60, 50, 10);
	...
	std::vector<Contour2D> FindContours2D::FindContoursUsingManualSelection()
	{
		...
		while (keyCode != 'q')
		{
			...
			if (keyCode != 'q' && this->manualLineSelected)
			{
				...
				std::cout << this->manualLineSelectionP1 << ", " << this->manualLineSelectionP2;
